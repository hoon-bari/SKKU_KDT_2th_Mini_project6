...
최종적으로는 하나의 ipynb파일이 올라가지만, 이걸 하기위해 4개의 ipynb파일을 만들어서 여러가지 실험을 해봤다.  
여기에는 그 실험했던 부분을 정리해서 적을 예정이다.  
또한 향후 ipynb파일을 모듈화해서 깃헙에 추가로 올릴 예정이다.  
...
2. 모든 학습은 코랩 유료버전에서 진행했다.  
   ...
   이걸로 알 수 있듯, 학습에는 돈과 시간만 있으면 뭐든지 할 수 있다.  
   역시 돈으로 안되는게 없다... 하긴 챗GPT는 한번 학습하는데 100억원 든다는데 나는 한 번 학습하는데 1000원이면 뭐... 나쁘지 않은듯.  
   최종 이 모델의 학습을 하는데 약 2시간이 걸렸는데, 더 많은 데이터와 시간, 그리고 돈을 투자하면 진짜 멋있는 번역기가 나올 것 같다.  
    
3. 추가로 50만 문장을 더 학습시켜서(문어체) 100만 문장에 fit한 모델로 측정을 하려고 했으나,
   모델 변경 없이 데이터만 model.fit을 해도 데이터 편향 문제가 생겼다.
   그래서 찾아보니 continual learning에 관한 연구가 있었는데,
